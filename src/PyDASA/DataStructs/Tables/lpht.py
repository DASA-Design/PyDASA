...
class LinearProbing(Generic[T]):
    ...
    *Nota:* la función MAD es: *h(k) = ((a*k + b) mod P) mod M*, donde *a* y *b* son números enteros aleatorios, *P* es un número primo y *M* es la capacidad de la tabla de hash.
    ...
    def _check_type(self, entry: MapEntry) -> bool:
        ...
        if key != EMPTY and value != EMPTY:
            if not isinstance(key, self._key_type):
                ...
                err_msg += f"for struct configured with: {self._key_type}"
                raise TypeError(err_msg)
            elif not isinstance(value, self._value_type):
                err_msg = f"Invalid value type: {type(value)} "
                err_msg += f"for struct configured with: {self._value_type}"
                raise TypeError(err_msg)
        # otherwise, the type is valid
        return True

    def _check_key_type(self, entry: MapEntry) -> bool:
        """*_check_key_type()* función propia de la estructura que revisa si el tipo de dato de la llave del registro (pareja llave-valor) que se desea agregar al *LinearProbing* es del mismo tipo contenido dentro de los *MapEntry* del *LinearProbing*.

        Args:
            element (T): elemento que se desea procesar en *LinearProbing*.

        Raises:
            TypeError: error si el tipo de dato de la llave que se desea agregar no es el mismo que el tipo de dato de las llaves que ya contiene el *LinearProbing*.

        Returns:
            bool: operador que indica si la llave del ADT *LinearProbing* es del mismo tipo que la llave que se desea procesar.
        """
        # TODO check usability of this function
        key = entry.get_key()
    ...
    def get(self, key: T) -> Optional[MapEntry]:
        """*get()* recupera el registro *MapEntry* cuya llave *key* sea igual a la que se encuentra dentro del *LinearProbing*, si no existe un registro con la llave, devuelve *None*.
    ...
    def remove(self, key: T) -> Optional[T]:
        """*remove()* elimina el registro *MapEntry* cuya llave *key* sea igual a la que se encuentra dentro del *LinearProbing*, si no existe un registro con la llave, genera un error.
