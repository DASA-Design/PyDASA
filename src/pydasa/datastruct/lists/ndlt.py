...
class Node(Generic[T]):
    ...
    def _error_handler(self, err: Exception) -> None:
        ...
        _cur_func = inspect.currentframe().f_code.co_name
        _cur_cxt = self.__class__.__name__
        error(_cur_cxt, _cur_func, err)

    def _check_type(self, elm: T) -> bool:
        """*_check_type()* function that checks if the type of the element is the same as the type of the *Node*.

        Args:
            elm (T): element to be processed in the *Node*.
