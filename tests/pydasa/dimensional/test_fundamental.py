...
class TestDimension(unittest.TestCase):
    ...
    def test_inheritance_from_validation(self) -> None:
        """Test that Dimension inherits from Foundation."""
        from pydasa.core.basic import Foundation, IdxBasis, SymBasis

        dim = Dimension(_sym="L", _fwk="PHYSICAL", _unit="m")

        assert isinstance(dim, Foundation)
        assert isinstance(dim, IdxBasis)
        assert isinstance(dim, SymBasis)
    ...
    def test_dimension_str_representation(self) -> None:
        """Test __str__ method inherited from Foundation."""
        data = self.test_data["PHYSICAL_DATA"]
        dim = Dimension(
            _idx=data["_idx"],
            _sym=data["_sym"],
        ...
        str_repr = str(dim)
        assert "Foundation(" in str_repr or "Dimension" in str_repr
